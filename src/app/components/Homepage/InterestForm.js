...
const InterestForm = (props) => {
    ...
    const onFieldChange = (e) => {
        ...
        if(e.target.id.substr(0,8) === 'checkbox'){
            switch(e.target.id){
                case 'checkbox1':
                    dispatch(setCheckbox1({type:'SET_CHECKBOX_1',interestForm: {checkbox1:!interestForm.checkbox1}}));
                    break;
                case 'checkbox2':
                    dispatch(setCheckbox2({type:'SET_CHECKBOX_2',interestForm: {checkbox2:!interestForm.checkbox1}}));
                    break;
                case 'checkbox3':
                    dispatch(setCheckbox3({type:'SET_CHECKBOX_3',interestForm: {checkbox3:!interestForm.checkbox3}}));
                    break;
                case 'checkbox4':
                    dispatch(setCheckbox4({type:'SET_CHECKBOX_4',interestForm: {checkbox4:!interestForm.checkbox4}}));
                    break;
                case 'checkbox5':
                    dispatch(setCheckbox5({type:'SET_CHECKBOX_5',interestForm: {checkbox5:!interestForm.checkbox5}}));
                    break;
                case 'checkbox6':
                    dispatch(setCheckbox6({type:'SET_CHECKBOX_6',interestForm: {checkbox6:!interestForm.checkbox6}}));
                    break;
            }
        }
        if(e.target.id !== currentField){
            ...
            switch(lastField){
                case 'first':
                    dispatch(setFirst({type:'SET_FIRST',interestForm: {first:tempText}}));
                    break;
                case 'last':
                    dispatch(setLast({type:'SET_LAST',interestForm: {last:tempText}}));
                    break;
                case 'phone':
                    dispatch(setPhone({type:'SET_PHONE',interestForm: {phone:tempText}}));
                    break;
                case 'email':
                    dispatch(setEmail({type:'SET_EMAIL',interestForm: {email:tempText}}));
                    break;
                case 'beckettId':
                    dispatch(setBeckettId({type:'SET_BECKETT_ID',interestForm: {beckettId:tempText}}));
                    break;
            }
        }
        if(e.target.id.substr(0,8) !== 'checkbox'){
                ...
                <InputGroup md={{ span: 2, offset: 2 }} className="justify-content-md-center">
                    <Col md='5'>
                        <input 
                            id='beckettId'
                            type='text' 
                            value={currentField === 'beckettId' ? tempText : interestForm.beckettId}
                            className='transparent-text-input border border-dark rounded-pill'
